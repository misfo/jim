(function() {
  ...
  define(function(require, exports, module) {
    ...
    regex = RegExp("^([vV])|(?:([1-9]\\d*)?(?:([iaoOIAC])|([DpPsxXu])|([cdy]{2})|(?:([cdy])?(" + motions.regex.source + ")?))?)$");
    return {
      execute: function() {
        var additionalLines, after, beforeLineEnding, column, command, continueBuffering, count, countMatch, deleteMotion, fullMatch, insertSwitch, lastRow, lineEnding, linewiseCommand, linewiseRegister, match, motionMatch, operator, registerValue, row, startingPosition, text, timesLeft, visualSwitch, wholeString, _ref, _ref2;
        ...
        fullMatch = match[0], visualSwitch = match[1], countMatch = match[2], insertSwitch = match[3], command = match[4], linewiseCommand = match[5], operator = match[6], motionMatch = 8 <= match.length ? __slice.call(match, 7) : [];
