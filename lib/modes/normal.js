(function() {
  define(function(require, exports, module) {
    ...
    regex = RegExp("^([iaoOIAC])|([vV])|(D)|(?:([1-9]\\d*)?(?:([pPsxXu])|([cdy]{2})|(?:([cdy])?([1-9]\\d*)?(" + motions.regex.source + ")?))?)$");
    return {
      execute: function() {
        var additionalLines, after, command, continueBuffering, count, countMatch, deleteCommand, deleteMotion, fullMatch, insertSwitch, linewiseCommand, match, motion, motionCount, motionCountMatch, motionObj, operator, row, startingPosition, text, timesLeft, visualSwitch, _ref;
        ...
        fullMatch = match[0], insertSwitch = match[1], visualSwitch = match[2], deleteCommand = match[3], countMatch = match[4], command = match[5], linewiseCommand = match[6], operator = match[7], motionCountMatch = match[8], motion = match[9];
