(function() {
  define(function(require, exports, module) {
    ...
    regex = RegExp("^([iaIAC])|([vV])|(D)|(?:([cdy])?(\\d*)(?:(" + motions.regex.source + ")|([[pPxXu]))?)$");
    ...
    return {
      execute: function() {
        var after, continueBuffering, deleteCommand, deleteMotion, fullMatch, match, motion, motionObj, multipliableCommand, numberPrefix, text, visualTransition;
        ...
        fullMatch = match[0], this.insertTransition = match[1], visualTransition = match[2], deleteCommand = match[3], this.operator = match[4], numberPrefix = match[5], motion = match[6], multipliableCommand = match[7];
        if (numberPrefix) {
