(function() {
  ...
  define(function(require, exports, module) {
    ...
    regex = RegExp("^([vV])|(?:([1-9]\\d*)?(?:([iaoOIAC])|([DpPsxXu])|(g?J)|(?:r([\\s\\S])?)|(cc|dd|yy)|(?:([cdy])?(" + motions.regex.source + ")?))?)$");
    return {
      execute: function() {
        var additionalLines, after, beforeLineEnding, column, command, continueBuffering, count, countMatch, deleteMotion, fullMatch, insertSwitch, joinCommand, lastRow, lineEnding, linewiseCommand, linewiseRegister, match, motionMatch, normalizeWhitespace, operator, registerValue, replacementChar, replacementText, row, startingPosition, text, timesLeft, visualSwitch, wholeString, _ref, _ref2;
        ...
        fullMatch = match[0], visualSwitch = match[1], countMatch = match[2], insertSwitch = match[3], command = match[4], joinCommand = match[5], replacementChar = match[6], linewiseCommand = match[7], operator = match[8], motionMatch = 10 <= match.length ? __slice.call(match, 9) : [];
        ...
        if (insertSwitch) {
          ...
          timesLeft = (count || 2) - 1;
          normalizeWhitespace = joinCommand === 'J';
          while (timesLeft--) {
            this.adaptor.selectLineEnding(normalizeWhitespace);
