(function() {
  ...
  define(function(require, exports, module) {
    var Motion, WORDRegex, c, dualChar, k, lastColumnWithChar, lastWORDRegex, lastWordRegex, moveBackWord, moveNextWord, moveWordEnd, nextColumnWithChar, simpleMotions, singleChar, toCharMotions, v, wordRegex;
    WORDRegex = function() {
      return /\S+/g;
    };
    wordRegex = function() {
    ...
    simpleMotions = {
      ...
      '/': new Motion({
        exclusive: true,
        move: function(jim, count) {
          var needle, timesLeft, _results;
          timesLeft = count != null ? count : 1;
          needle = prompt("Find:");
          _results = [];
          while (timesLeft--) {
            _results.push(jim.adaptor.find(needle));
          }
          return _results;
        }
      }),
      n: new Motion({
        ...
        move: function(jim, count) {
          ...
          _results = [];
          while (timesLeft--) {
            _results.push(jim.adaptor.findNext());
          }
          return _results;
        }
      }),
      N: new Motion({
        ...
        move: function(jim, count) {
          ...
          _results = [];
          while (timesLeft--) {
            _results.push(jim.adaptor.findPrevious());
          }
          return _results;
        }
      })
    ...
    singleChar = [];
    dualChar = [];
    for (k in simpleMotions) {
      if (!__hasProp.call(simpleMotions, k)) continue;
      v = simpleMotions[k];
      switch (k.length) {
        case 1:
          singleChar.push(k);
