(function() {
  define(function(require, exports, module) {
    var Jim, WORDRegex, actions, adaptor, atLineEnd, beyondLineEnd, isntCharacterKey, jim, lastWORDRegex, lastWordRegex, moveSelection, navigateBackWord, navigateNextWord, navigateWordEnd, wordRegex;
    Jim = require('jim/jim');
    jim = new Jim();
    ...
    moveSelection = function(motionAction, env, args) {
      var lead;
    ...
    actions = {
      ...
      navigateLeft: function(env, args) {
        var times, _ref, _results;
        times = (_ref = args.times) != null ? _ref : 1;
        _results = [];
        while (times-- && env.editor.selection.selectionLead.getPosition().column > 0) {
          _results.push(env.editor.selection.moveCursorLeft());
        }
        return _results;
      },
      navigateRight: function(env, args) {
        ...
        times = (_ref = args.times) != null ? _ref : 1;
        _results = [];
        while (times-- && !atLineEnd(env.editor)) {
          _results.push(env.editor.selection.moveCursorRight());
        }
        return _results;
      },
      ...
      deleteSelection: function(env, args) {
        if (env.editor.selection.isEmpty() || !env.editor.selection.isBackwards()) {
          if (!beyondLineEnd(env.editor)) {
      ...
      selectLeft: function(env, args) {
        return moveSelection(actions.navigateLeft, env, args);
      },
      selectRight: function(env, args) {
        return moveSelection(actions.navigateRight, env, args);
      },
      selectLine: function(env, args) {
        return env.editor.selection.selectLine();
      },
      selectBackWORD: function(env, args) {
        return moveSelection(actions.navigateBackWORD, env, args);
      },
      selectWORDEnd: function(env, args) {
        return moveSelection(actions.navigateWORDEnd, env, args);
      },
      selectNextWORD: function(env, args) {
        return moveSelection(actions.navigateNextWORD, env, args);
      },
      selectBackWord: function(env, args) {
        return moveSelection(actions.navigateBackWord, env, args);
      },
      ...
      yankSelection: function(env, args) {
        ...
        if (env.editor.selection.isBackwards()) {
          ...
          return env.editor.moveCursorTo(start.row, start.column);
        }
      }
    };
