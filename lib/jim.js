(function() {
  ...
  modes = {
    normal: {
      regex: /^([iIAC])|([D])|(?:(\d*)(?:([hjkl])|(G))?)/,
      parse: function(buffer) {
        var args, changeToMode, deleteCommand, fullMatch, go, insertTransition, match, method, movement, numberPrefix;
        ...
        fullMatch = match[0], insertTransition = match[1], deleteCommand = match[2], numberPrefix = match[3], movement = match[4], go = match[5];
        method = 'doNothing';
        args = {};
        changeToMode = null;
        if (insertTransition) {
